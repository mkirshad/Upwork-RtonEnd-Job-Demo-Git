...
## Socialista Event Platform Demo
- Visit `/socialista` to view the Socialista showcase describing the React Native attendee app, Next.js web experience, organizer cockpit, and admin command center expected in the Upwork posting.
- Copy, events, launch milestones, and stats are centralized in [`src/data/socialista.ts`](src/data/socialista.ts) so you can refresh the narrative quickly before sharing with a client.
- The UI lives in [`src/views/demo/SocialistaShowcase.tsx`](src/views/demo/SocialistaShowcase.tsx) and inherits the same storefront layout system, ensuring the demo matches the rest of the IrshadOS storytelling assets.
- Buttons on the Passion Atelier landing page now link directly to this case study for quick navigation.
